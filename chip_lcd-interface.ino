...
char aperture_sym = 'W'; //Initialized to K for f2.0
...
int index_ap = 0; //index holder for aperture array
...
void setup() {
  ...
  lcd.begin(16,2);
  lcd.print("Canon EOS66 35mm");
  lcd.setCursor(0,1);
  lcd.print("Film Camera : RaE Industries");
  ...
  if((butn_sel < butn_threshold)||(butn_up < butn_threshold)||(butn_down < butn_threshold)){ 
    ...
    while((millis()-times)<timeout){ //Enter timeout period
      ...
      if(analogRead(A2)<butn_threshold){ //Mode button pressed
        ...
        while(analogRead(A2)<butn_threshold){
          delay(10);
        }
